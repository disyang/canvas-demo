...
### 3、`Canvas`图片放大镜功能，本地测试`Canvas`上下文api getImageData浏览器会报跨域问题，Firefox不存在这个问题。样式如下：  
